...
const getSellers = async () => {
    try {
        ...
        console.log("Full seller response:", response);
...
const addSeller = async (sellerData) => {
    try {
        const response = await Axios.post(BaseURL + "seller", sellerData, { headers });
        if (response.status === 201) {
...
const updateSeller = async (sellerId, sellerData) => {
    try {
        const response = await Axios.patch(BaseURL + "seller/" + sellerId, sellerData, { headers });
        if (response.status === 200) {
...
const deleteSeller = async (sellerId) => {
    try {
        const response = await Axios.delete(BaseURL + "seller/" + sellerId, { headers });
        if (response.status === 200) {
